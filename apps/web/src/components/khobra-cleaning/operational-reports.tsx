...
export function OperationalReports({ bookings }: { bookings: any[] }) {
  const now = new Date()
  ...
  const scoped = useMemo(() => bookings.filter(booking => {
    const value = day(booking.scheduledDate)
    return (!from || value >= from) && (!to || value <= to)
  }), [bookings, from, to])
  const completed = useMemo(() => scoped.filter(booking => booking.status === 'completed'), [scoped])
  ...
  const assignmentRows = useMemo<Row[]>(() => scoped.flatMap(booking => (booking.assignments || []).map((assignment: any) => ({
  ...
  return <Card className="border-0 shadow-sm">
    ...
    <CardContent>
      <Tabs defaultValue="bookings">
        ...
        <TabsContent value="completed" className="space-y-3"><div className="flex justify-end">{actions(`Completed Bookings ${from} to ${to}`, bookingRows.filter(row => row.Status === 'completed'))}</div>{bookingTable(bookingRows.filter(row => row.Status === 'completed'))}</TabsContent>
