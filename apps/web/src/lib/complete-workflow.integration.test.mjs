...
test('complete operational workflow passes through real API routes and persistence', async () => {
  ...
  try {
    ...
    assert.equal(variantResult.data.assignments.length, 1, 'System must assign exactly the requested cleaner count')

    const partialDate = new Date(Date.now() + 5 * 86_400_000).toISOString().slice(0, 10)
    const partialResult = await request('/bookings', auth.customer, 'POST', { customerId: customer.id, serviceId: service.id, serviceIds: [service.id], serviceOptions: [{ serviceId: service.id, withMaterials: false }], scheduledDate: partialDate, startDate: partialDate, endDate: partialDate, startTime: '09:00', endTime: '11:00', employeeCount: 2, preferredEmployeeIds: [cleanerOne.id], bookingType: 'one_time' })
    assert.equal(partialResult.response.status, 201, `Customer partial cleaner selection must succeed: ${JSON.stringify(partialResult.data)}`)
    ids.bookingIds.push(partialResult.data.id)
    assert.equal(partialResult.data.assignments.length, 2, 'System must fill every unselected cleaner slot')
    assert.ok(partialResult.data.assignments.some(assignment => assignment.employeeId === cleanerOne.id), 'System must preserve the cleaner selected by the customer')
    await db.booking.updateMany({ where: { id: { in: [variantResult.data.id, partialResult.data.id] } }, data: { status: 'cancelled', deletedAt: new Date() } })
    ...
    assert.ok(booking.assignments.every(assignment => Number(assignment.actualHours) >= 2), 'Every completed assignment must honor the two-hour minimum')
    ...
    assert.equal(Number(booking.invoices[0].totalAmount), Number(booking.netAmount), 'Completed invoice must preserve the amount accepted when the booking was placed')
