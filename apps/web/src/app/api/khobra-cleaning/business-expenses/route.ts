...
export async function POST(req: NextRequest) {
  try {
    ...
    return NextResponse.json(await db.businessExpense.create({ data: { ...data, currency: data.currency.toUpperCase(), tenantId: auth.session.tenantId, createdBy: auth.session.userId } }), { status: 201 })
  } catch (error: any) {
