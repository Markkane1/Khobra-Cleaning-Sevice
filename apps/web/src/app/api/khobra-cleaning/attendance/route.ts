...
const attendanceService = new AttendanceService(new PrismaAttendanceRepository(db))
...
export async function GET(req: NextRequest) {
  try {
    ...
    const records = await attendanceService.getAttendances(auth.session.tenantId)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const record = await attendanceService.createAttendance(auth.session.tenantId, { ...validated, employeeId: cleaner.id })
