...
For local Android development, run the web app, forward the emulator port, and explicitly set the local URL:
...
When `CAPACITOR_SERVER_URL` is unset, the wrapper uses its bundled offline page instead of silently shipping a cleartext localhost target.
