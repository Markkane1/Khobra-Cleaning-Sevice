...
import { Alert, AppState, FlatList, Modal, Pressable, ScrollView, StyleSheet, Text, View, SafeAreaView } from 'react-native'
...
type Employee = { id: string; employeeCode: string; user?: { name: string; email: string; phone?: string }; skills: string; baseSalary: number; status: string; _count?: { assignments: number }; averageRating?: number; ratingCount?: number }

const emptyForm = { name: '', email: '', phone: '', skills: '', baseSalary: '0', status: 'active', temporaryPassword: '' }
...
export function EmployeesScreen({ session, onBack }: { session: Session; onBack?: () => void }) {
  ...
  const load = (silent = false) => {
    ...
    request<Employee[]>('/api/khobra-cleaning/employees', {}, session.token)
      .then(setEmployees)
      .catch(() => { if (!silent) Alert.alert('Error', 'Could not load employees.') })
  ...
  const save = async () => {
    ...
    try {
      ...
      await request('/api/khobra-cleaning/employees', {
  ...
  const remove = (id: string) => {
    Alert.alert('Remove Employee', 'Are you sure?', [
      ...
      { text: 'Remove', style: 'destructive', onPress: async () => {
        try {
          await request(`/api/khobra-cleaning/employees?id=${id}`, { method: 'DELETE' }, session.token)
          load()
        } catch (e) {
  ...
  const openEdit = (e: Employee) => {
    setEditId(e.id)
    setForm({ name: e.user?.name || '', email: e.user?.email || '', phone: e.user?.phone || '', skills: e.skills || '', baseSalary: String(e.baseSalary || 0), status: e.status || 'active', temporaryPassword: '' })
    setFormOpen(true)
  }
  ...
  return <View style={styles.screen}>
    <View style={styles.header}>
      ...
      <PageHeading title="Cleaners" subtitle="Workforce management and skills tracking" action={<Pressable accessibilityRole="button" accessibilityLabel="Add cleaner" onPress={openNew} style={styles.addButton}><Ionicons name="add" size={24} color="#fff" /></Pressable>} />
    </View>
      ...
      renderItem={({ item: e }) => <View style={styles.card}>
        <View style={styles.cardTop}>
          ...
          <View style={styles.cardInfo}>
            ...
            {e.user?.phone ? <View style={styles.metaRow}><Ionicons name="call-outline" size={12} color={palette.muted} /><Text style={styles.metaText}>{e.user.phone}</Text></View> : null}
          </View>
        ...
        <View style={styles.actions}>
          ...
          <View style={styles.actionRow}>
            ...
            <Pressable accessibilityRole="button" accessibilityLabel={`Edit ${e.user?.name || 'cleaner'}`} style={styles.iconButton} onPress={() => openEdit(e)}><Ionicons name="pencil" size={18} color={palette.primary} /></Pressable>
            <Pressable accessibilityRole="button" accessibilityLabel={`Delete ${e.user?.name || 'cleaner'}`} style={styles.iconButton} onPress={() => remove(e.id)}><Ionicons name="trash" size={18} color={palette.danger} /></Pressable>
          </View>
        </View>
      </View>}
    />}
    ...
    <Modal visible={formOpen} animationType="slide" presentationStyle="formSheet">
      <SafeAreaView style={styles.modalScreen}>
        <View style={styles.modalHeader}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="Close cleaner form" onPress={() => setFormOpen(false)} style={styles.closeBtn}><Ionicons name="close" size={24} color={palette.ink} /></Pressable>
        </View>
...
const styles = StyleSheet.create({
  ...
  skillText: { fontSize: 12, color: '#065f46', fontWeight: '600' },
  ...
  statText: { fontSize: 12, fontWeight: '600', color: palette.ink },
