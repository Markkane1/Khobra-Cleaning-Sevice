...
export function InvoicesScreen({ session, onBack }: { session: Session; onBack?: () => void }) {
  ...
  const load = () => {
    setLoading(true)
    Promise.all([
      request<Invoice[]>('/api/khobra-cleaning/invoices', {}, session.token),
      request<Payment[]>('/api/khobra-cleaning/payments', {}, session.token),
  ...
  useEffect(() => subscribeRealtime(session.token, ['invoice:created', 'invoice:updated', 'payment:created', 'payment:updated'], load), [session.token])

  const saveInvoice = async () => {
    if (!formCustomer || !formAmount) return Alert.alert('Validation', 'Please select customer and enter amount.')
    setSaving(true)
    try {
      await request('/api/khobra-cleaning/invoices', {
        method: 'POST',
  ...
  const savePayment = async () => {
    ...
    setSaving(true)
    try {
      await request('/api/khobra-cleaning/payments', {
        method: 'POST',
    ...
    <View style={styles.header}>
      ...
      <PageHeading title="Finance" subtitle="Invoices & Payments" action={
        <View style={{flexDirection: 'row', gap: 8}}>
          <Pressable accessibilityRole="button" accessibilityLabel="Record payment" onPress={() => setPayFormOpen(true)} style={[styles.addButton, {backgroundColor: palette.primaryDark}]}><Ionicons name="card-outline" size={24} color="#fff" /></Pressable>
          <Pressable onPress={() => {
            setFormCustomer('')
            setFormAmount('')
            setFormStatus('issued')
            setFormOpen(true)
          }} accessibilityRole="button" accessibilityLabel="Generate invoice" style={styles.addButton}><Ionicons name="add" size={24} color="#fff" /></Pressable>
        </View>
      } />
    </View>
    ...
    <Modal visible={formOpen} animationType="slide" presentationStyle="formSheet">
      <SafeAreaView style={styles.modalScreen}>
        <View style={styles.modalHeader}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="Close invoice form" onPress={() => setFormOpen(false)} style={styles.closeBtn}><Ionicons name="close" size={24} color={palette.ink} /></Pressable>
        </View>
    ...
    <Modal visible={payFormOpen} animationType="slide" presentationStyle="formSheet">
      <SafeAreaView style={styles.modalScreen}>
        <View style={styles.modalHeader}>
          ...
          <Pressable accessibilityRole="button" accessibilityLabel="Close payment form" onPress={() => setPayFormOpen(false)} style={styles.closeBtn}><Ionicons name="close" size={24} color={palette.ink} /></Pressable>
        </View>
        <ScrollView contentContainerStyle={styles.formBody}>
          <View style={styles.formGroup}>
...
const styles = StyleSheet.create({
  ...
  kpiLabel: { fontSize: 12, fontWeight: '700', color: palette.muted, marginBottom: 4 },
  ...
  tabText: { fontSize: 14, fontWeight: '700', color: palette.ink },
  ...
  invoiceNo: { fontSize: 14, fontWeight: '800', fontFamily: 'Courier New', color: palette.muted },
  ...
  amountLabel: { fontSize: 14, color: palette.muted, fontWeight: '600' },
  amountValue: { fontSize: 18, fontWeight: '800', color: palette.ink },
  progressBox: {},
  progressHeader: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 },
  progressLabel: { fontSize: 12, color: palette.muted, fontWeight: '600' },
