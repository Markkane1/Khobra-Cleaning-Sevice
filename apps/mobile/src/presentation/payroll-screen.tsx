...
export function PayrollScreen({ session, onBack }: { session: Session; onBack?: () => void }) {
  ...
  const load = () => {
    setLoading(true)
    fetch(`${apiBaseUrl}/api/khobra-cleaning/payroll`, { headers: { Authorization: `Bearer ${session.token}` } })
      .then(r => r.json())
  ...
  const approve = async (rec: PayrollRecord) => {
    try {
      const res = await fetch(`${apiBaseUrl}/api/khobra-cleaning/payroll`, {
        method: 'PUT',
  ...
  return <View style={styles.screen}>
    <View style={styles.header}>
      ...
      <PageHeading title="Payroll" subtitle={summary?.month ? `Processing for ${summary.month}` : 'Employee salary management'} action={<Pressable onPress={approveAll} style={styles.approveAllBtn}><Ionicons name="checkmark-done" size={24} color="#fff" /></Pressable>} />
    </View>
...
const styles = StyleSheet.create({
  ...
  summaryLabel: { fontSize: 11, color: palette.muted, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  empCode: { fontSize: 13, color: palette.muted, fontWeight: '400' },
  ...
  salItem: { width: '47%' },
  salLabel: { fontSize: 11, color: palette.muted, marginBottom: 2 },
  salVal: { fontSize: 14, fontWeight: '600', color: palette.ink },
  
  approveBtn: { backgroundColor: palette.primarySoft, padding: 12, borderRadius: 10, marginTop: 14, alignItems: 'center' },
