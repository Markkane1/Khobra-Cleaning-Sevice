...
export function DriverExpensesScreen({ session }: { session: Session }) {
  ...
  const reset = () => { setCategory('petrol'); setTypeDetail(''); setAmount('0'); setExpenseDate(localDateValue()); setNotes('') }
  ...
  return <>
    <FlatList contentContainerStyle={styles.list} data={expenses} keyExtractor={item => item.id}
      ListHeaderComponent={<PageHeading title="Expenses" subtitle="Submit petrol, repair and other transport costs." action={<Pressable accessibilityRole="button" accessibilityLabel="Add driver expense" onPress={() => setOpen(true)} style={styles.add}><Ionicons name="add" size={22} color="#fff" /></Pressable>} />}
...
const styles = StyleSheet.create({
  ...
  card: { flexDirection: 'row', alignItems: 'flex-start', gap: 11, padding: 15, backgroundColor: palette.surface, borderWidth: 1, borderColor: palette.border, borderRadius: 18, ...cardShadow }, icon: { width: 42, height: 42, borderRadius: 13, backgroundColor: palette.primarySoft, alignItems: 'center', justifyContent: 'center' }, body: { flex: 1, gap: 3 }, title: { color: palette.ink, fontWeight: '800', fontSize: 14 }, meta: { color: palette.muted, fontSize: 12 }, notes: { color: palette.inkSoft, fontSize: 12 }, status: { borderRadius: 999, paddingHorizontal: 9, paddingVertical: 5 }, statusText: { color: palette.inkSoft, fontSize: 12, fontWeight: '900', textTransform: 'uppercase' },
  backdrop: { flex: 1, backgroundColor: 'rgba(15,23,42,.5)', justifyContent: 'flex-end' }, modal: { maxHeight: '90%', backgroundColor: palette.surface, borderTopLeftRadius: 24, borderTopRightRadius: 24, paddingTop: 22 }, modalTitle: { color: palette.ink, fontSize: 20, fontWeight: '800', paddingHorizontal: 22 }, form: { padding: 22, gap: 9, paddingBottom: 36 }, label: { color: palette.ink, fontWeight: '700', fontSize: 14, marginTop: 3 }, categories: { gap: 7 }, category: { minHeight: 44, justifyContent: 'center', borderWidth: 1, borderColor: palette.border, borderRadius: 999, paddingHorizontal: 12 }, categoryActive: { backgroundColor: palette.primary, borderColor: palette.primary }, categoryText: { color: palette.muted, fontSize: 12, fontWeight: '700' }, categoryTextActive: { color: '#fff' }, input: { minHeight: 48, borderWidth: 1, borderColor: palette.border, backgroundColor: palette.surfaceMuted, borderRadius: 12, padding: 12, color: palette.ink, fontSize: 14 }, notesInput: { minHeight: 80, textAlignVertical: 'top' }, actions: { flexDirection: 'row', justifyContent: 'flex-end', gap: 9, marginTop: 8 }, cancel: { minHeight: 44, justifyContent: 'center', borderWidth: 1, borderColor: palette.border, borderRadius: 11, paddingHorizontal: 15 }, cancelText: { color: palette.ink, fontSize: 14, fontWeight: '700' }, submit: { minHeight: 44, justifyContent: 'center', backgroundColor: palette.primary, borderRadius: 11, paddingHorizontal: 15 }, submitText: { color: '#fff', fontSize: 14, fontWeight: '800' },
})
